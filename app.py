...
3. Infrastructure Project Status
4. E-Governance Implementation Progress
5. Key Government Programs
6. Public Grievance Redressal
7. Health and Education Initiatives  
8. Challenges and Mitigation Strategies
9. District-wise Performance Analysis
10. Recommendations for the Next Quarter
11. Future Outlook and Strategic Plan
...
response = ollama.chat(model=OLLAMA_MODEL, messages=[  
    {"role": "system", "content": "You are an AI report generator specialized in government department reporting with expertise in data analysis and visualization. Create clear, concise, and actionable reports that highlight key insights and trends."},  
...
def create_visualizations():
    ...
    if has_additional_data:
        ...
        # Exclude 'Total' row from calculations
        time_df = time_series_df[:-1].copy()
        ...
        resolution_rate = (time_series_df['Grievances_Resolved'] / 
                         time_series_df['Grievances_Received'] * 100)[:-1]  # Exclude total
        ...
        x = np.arange(len(time_series_df)-1)  # Exclude total
        width = 0.35
        
        received = ax1.bar(x - width/2, time_series_df['Grievances_Received'][:-1], 
                         width, label='Received', color='#3498db')
        resolved = ax1.bar(x + width/2, time_series_df['Grievances_Resolved'][:-1], 
        ...
        # Set x-axis ticks
        plt.xticks(x, time_series_df['Month'][:-1], rotation=45)
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 6, "April 2023 - March 2024", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
...
if has_additional_data:
    ...
    data = [[str(sectoral_df.loc[sectoral_df['Sector'] == 'E-Governance', 'Projects_Count'].values[0]) if 'E-Governance' in sectoral_df['Sector'].values else "N/A", 
             f"{time_series_df['OnlineServiceHits'][:-1].mean():.0f}",
